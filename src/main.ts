...
try {
    ...
    [lua, lua2].forEach((lua) => {
      lua.global.set('color', (r:number, g:number, b:number) => {
        if(gctx != null){
          gctx.fillStyle = `rgb(${r},${g},${b})`
        }
      })
      lua.global.set('print', (s:string, x:number, y:number) => {
        if(gctx != null){
          gctx.fillText(s, x, y)
        }
      })
      lua.global.set('measureText', (s:string) => {
        if(gctx != null){
    ...
    luaKeydown = async (k:string) => {
      ...
      draw()
      const onKeyHandler2 = lua2.global.get('onKeyHandler')
      const setPos = lua2.global.get('setPos')
      onKeyHandler2(k, (key:string) => {
        const onCharHandler = lua.global.get('onCharHandler')
        onCharHandler(key, setPos)
      })
    }
    // Run a lua string
    await lua.doString(`
    ...
    function draw(setPos)
      ...
      for i, l in pairs(lines) do
        ...
        for p, c in utf8.codes(l) do
          ...
          color(0,0,0)
          print(uc, offset + px, py)
          px = px + measureText(uc)
          j = j + 1
        end
        if i == y and j == x then
          -- draw cursor
    ...
    function onCharHandler(k, setPos)
      debug("keydown: " .. k)
      local key = k
      if key == "Enter" then
        local line = lines[y]
    ...
    await lua2.doString(`
      ...
      function onKeyHandler(k, onCharHandler)
      ...
      function draw()
        ...
        local w = measureText(candidate)
        color(0,0,255)
        fillrect(cx, cy, w + 12, 12)
        color(255,255,255)
        print(candidate, cx, cy)
        local maxW = 0
        for i=1, #results do
          local w = measureText(results[i])
          if maxW < w then
            maxW = w
          end
        end
